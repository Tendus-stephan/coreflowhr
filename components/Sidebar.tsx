...
import { LayoutDashboard, Briefcase, Users, Calendar, Settings, LogOut, User as UserIcon, FileText, ChevronLeft, ChevronRight, Building2, BarChart3 } from 'lucide-react';
...
const Sidebar: React.FC = () => {
  ...
  return (
    <div className={`${isExpanded ? 'w-64' : 'w-20'} bg-white border-r border-gray-100 flex flex-col fixed top-0 left-0 bottom-0 z-20 transition-all duration-150 hidden md:flex`} style={{ overflow: 'hidden' }}>
      ...
      <nav className={`flex-1 ${isExpanded ? 'px-3' : 'px-2'} space-y-1 overflow-y-auto`} style={{ overflowX: 'hidden' }}>
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`flex items-center ${isExpanded ? 'gap-3 px-3' : 'justify-center px-2'} py-2.5 rounded-lg transition-colors duration-100 group relative ${
                isActive
            ...
            >
              <item.icon size={15} className={`transition-colors flex-shrink-0 ${isActive ? 'text-gray-900' : 'text-gray-400 group-hover:text-gray-600'}`} />
              {isExpanded && (
      ...
      <div className={`${isExpanded ? 'p-3' : 'p-2'} border-t border-gray-100 relative flex-shrink-0 bg-white`} ref={profileRef} style={{ marginTop: 'auto' }}>
        {/* Profile Button */}
        <div 
