...
export const api = {
    ...
    offers: {
        ...
        send: async (offerId: string, _options?: { requireEsignature?: boolean }): Promise<Offer> => {
            ...
            if (candError || !candidate?.email) throw new Error('Candidate not found or missing email address.');

            // Look up job for company name
            const { data: job } = await supabase
                .from('jobs').select('title, company').eq('id', offer.jobId).single();
            const companyName = (job as any)?.company || 'Our Company';

            // Generate a secure 64-char hex token and set 60-day expiry
            ...
            const currencySymbol = (offer.salaryCurrency || 'USD') === 'USD' ? '$' : (offer.salaryCurrency || '');
            const salaryNum = offer.salaryAmount ? offer.salaryAmount.toLocaleString() : '';
            const periodLabel = offer.salaryPeriod === 'yearly' ? 'per year' : offer.salaryPeriod === 'monthly' ? 'per month' : 'per hour';
            const salaryText = offer.salaryAmount ? `${currencySymbol}${salaryNum} ${periodLabel}` : 'To be discussed';
            const startDateText = offer.startDate
                ? new Date(offer.startDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
                : 'To be determined';
            const expiryRow = offer.expiresAt
                ? `<tr><td style="padding:8px 12px;background:#f9fafb;border:1px solid #e5e7eb;font-weight:600;">Offer Expires</td><td style="padding:8px 12px;border:1px solid #e5e7eb;">${new Date(offer.expiresAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</td></tr>` : '';
            const benefitsHtml = (offer.benefits && offer.benefits.length > 0)
                ? `<ul style="margin:8px 0 0 0;padding-left:20px;color:#374151;">${offer.benefits.map((b: string) => `<li>${b}</li>`).join('')}</ul>`
                : '<p style="color:#6b7280;margin:4px 0;">Standard benefits package</p>';

            const emailContent = `
<p>Dear ${candidate.name},</p>
<p>We are pleased to extend an offer of employment for the position of <strong>${offer.positionTitle}</strong> at <strong>${companyName}</strong>.</p>
<table style="width:100%;border-collapse:collapse;margin:20px 0;font-size:14px;">
  <tr><td style="padding:8px 12px;background:#f9fafb;border:1px solid #e5e7eb;font-weight:600;width:40%;">Position</td><td style="padding:8px 12px;border:1px solid #e5e7eb;">${offer.positionTitle}</td></tr>
...
<p>Best regards,<br><strong>${companyName}</strong></p>`;
            ...
            try {
                ...
                if (!stageUpdateError && oldStage && oldStage !== 'Offer' && candidateName) {
                    try { const { logCandidateMoved } = activityLogger; await logCandidateMoved(candidateName, oldStage as CandidateStage, CandidateStage.OFFER); } catch { /* non-critical */ }
                }
        ...
        acceptByToken: async (token: string, response?: string): Promise<Offer> => {
            ...
            return {
                ...
                positionTitle: offerRow.position_title,
                startDate: offerRow.start_date || undefined,
                salaryAmount: offerRow.salary_amount ? parseFloat(offerRow.salary_amount) : undefined,
                salaryCurrency: offerRow.salary_currency || 'USD',
                salaryPeriod: offerRow.salary_period || 'yearly',
                benefits: offerRow.benefits || undefined,
                notes: offerRow.notes || undefined,
                status: offerRow.status as Offer['status'],
                sentAt: offerRow.sent_at || undefined,
                viewedAt: offerRow.viewed_at || undefined,
                respondedAt: offerRow.responded_at || undefined,
                expiresAt: offerRow.expires_at || undefined,
                response: offerRow.response || undefined,
                requireEsignature: offerRow.require_esignature || false,
                negotiationHistory: offerRow.negotiation_history || undefined,
...
<p style="font-size:12px;color:#6b7280">This link expires in 7 days. If you did not expect this, please contact your administrator.</p>`;
        ...
        getApprovalByToken: async (token: string): Promise<{
            ...
            return {
                ...
                recruiterName: d.recruiter_name ?? null,
            };
        },
