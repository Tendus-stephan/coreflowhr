...
-- Service role / Edge Functions need to read and delete by user_id (e.g. after confirmation)
-- Already covered: no policy for UPDATE; we only INSERT (from client), SELECT/DELETE from client or from Edge Function with service role (bypasses RLS when using service role).
